...
export class GameController {
  ...
  async playGame(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!authHeader?.startsWith("Bearer ")) {
        return next(
          createHttpError(401, "Invalid token format. Expected: Bearer <token>")
        );
      }
      ...
      return res
        .status(201)
        .json(successResponse({ token }, "Game token created sucessfully"));
    } catch (error) {
      next(error);
    }
  }
  ...
  async createGame(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res
        .status(201)
        .json(successResponse(game, "Game created successfully"));
    } catch (error) {
      next(error);
    }
  }
  ...
  async updateGame(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const hasFileUpdates =
        files && (files["thumbnail"]?.[0] || files["payout"]?.[0]);

      if (!hasBodyUpdates && !hasFileUpdates) {
        throw createHttpError.BadRequest(
          "No updates provided - please provide either field updates or files to update"
        );
      }
      ...
      if (
        updateData.status &&
        !Object.values(GameStatus).includes(updateData.status)
      ) {
        throw createHttpError.BadRequest("Invalid status value");
      }
      ...
      return res
        .status(200)
        .json(successResponse(game, "Game updated successfully"));
    } catch (error) {
      next(error);
    }
  }
  ...
  async getGamePayouts(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res
        .status(200)
        .json(successResponse(payouts, "Payouts fetched successfully"));
    } catch (error) {
      next(error);
    }
  }
  ...
  async activateGamePayout(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res
        .status(200)
        .json(successResponse(payout, "Payout activated successfully"));
    } catch (error) {
      next(error);
    }
  }
  ...
  async reorderGames(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (
        !reorderData.every(
          (item) => item.gameId && typeof item.order === "number"
        )
      ) {
        throw createHttpError.BadRequest(
          "Each item must have gameId and order"
        );
      }
      ...
      return res
        .status(200)
        .json(successResponse(null, "Games reordered successfully"));
    } catch (error) {
      next(error);
    }
  }
