...
export class GameService {
  ...
  async createGame(
    ...
    payout?: { content: any; filename: string }
  ): Promise<IGame> {
    ...
    try {
      ...
      // 4. Upload thumbnail
      const thumbnailUploadResult = await this.cloudinaryService.uploadImage(
        thumbnailBuffer
      );
  ...
  async getGames(
    ...
    const { search, from, to, status, type, category, tag, ...otherFilters } =
      filters;
    const query: any = {
      status: { $ne: GameStatus.DELETED },
      ...otherFilters,
  ...
  async getGame({
    ...
    if (id) {
      ...
      throw createHttpError.BadRequest(
        "At least one identifier (id, tag, slug, name) is required"
      );
    }
  ...
  async updateGame(
    ...
    try {
      ...
      if (payout && payout.content && payout.filename) {
        const newPayoutId = await this.payoutService.createPayout(
          existingGame._id,
          payout,
          session
        );
  ...
  async activateGamePayout(gameId: string, payoutId: string): Promise<any> {
    if (
      ...
      !mongoose.Types.ObjectId.isValid(payoutId)
    ) {
  ...
  async reorderGames(
    reorderData: { gameId: string; order: number }[]
  ): Promise<void> {
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      const invalidIds = reorderData.filter(
        ({ gameId }) => !mongoose.Types.ObjectId.isValid(gameId)
      );
      if (invalidIds.length > 0) {
        throw createHttpError.BadRequest(
          `Invalid game IDs: ${invalidIds.map((i) => i.gameId).join(", ")}`
        );
      }
      ...
      if (existingGames.length !== gameIds.length) {
        const missingIds = gameIds.filter(
          (id) => !existingGames.find((game) => game._id.toString() === id)
        );
  ...
  async uploadGames(games: IGame[]): Promise<{
    total: number;
    created: number;
    skipped: number;
    errors: string[];
      ...
      for (const game of games) {
        try {
          ...
          if (error instanceof Error) {
            result.errors.push(
              `Failed to import ${game.name}: ${error.message}`
            );
          } else {
  ...
  async downloadGames(): Promise<IGame[]> {
    const games = await Game.find({ status: { $ne: GameStatus.DELETED } })
      .populate("payout")
