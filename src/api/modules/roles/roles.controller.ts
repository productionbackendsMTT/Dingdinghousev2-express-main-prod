...
class RoleController {
  ...
  async updateRole(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (
        status &&
        ![RoleStatus.ACTIVE, RoleStatus.INACTIVE].includes(status)
      ) {
      ...
      if (descendants !== undefined) {
        if (!operation) {
          throw createHttpError.BadRequest(
            "Operation is required when updating descendants"
          );
        }
      ...
      res
        .status(200)
        .json(successResponse(role, `Role ${role.name} updated successfully`));
    } catch (error) {
      next(error);
    }
  }
  ...
  async getRoleById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res
        .status(200)
        .json(successResponse(role, "Role retrieved successfully"));
    } catch (err) {
      next(err);
    }
  }
