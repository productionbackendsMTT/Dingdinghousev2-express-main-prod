...
import { GameFactory } from './common/system/game-factory';
...
let currentGame: BaseSlotGame | BaseKenoGame;

(async () => {
    const game = await GameFactory.create(defaultGameConfig);
    if (!game) {
        throw new Error('Failed to create game instance');
    }
    if (game instanceof BaseSlotGame) {
        currentGame = game;
    } else if (game instanceof DefaultKenoGame) {
        currentGame = game;
    }
})();
